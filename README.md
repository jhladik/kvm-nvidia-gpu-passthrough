...
1. Find where your `gamecontrollerdb.txt` is stored. On Mac OS, it is in `~/Library/Caches/Moonlight Game Streaming Project/Moonlight/gamecontrollerdb.txt`. The following exerpt will map the joystick axes and buttons to the Xbox Controller which Moonlight can forward. You can then assign the Xbox Controller in your flight simulator settings.
```
